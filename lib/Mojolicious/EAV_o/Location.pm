...
package Kernel::System::EAV::Location;
...
sub GetLocationsByOffice {
    ...
    my $r = $Self->{DBObject}->SelectAllHR( $sql ) || [];
...
sub GetTree {
    ...
    my $AskedNodes = {
        map {
            ...
            ( $_->{id}, $_ )
        } @{ $Self->{DBObject}->SelectAllHR( $AskedNodesSQL ) }
    };
    ...
    my $MixedTree = $Self->{DBObject}->SelectAllHR( $MixedTreeSQL );
    my $MixedTreeAsHash = {
        map {
...
sub SearchLocations {
    ...
    if ( defined( $Params->{Term} ) ) {
        $Term = lc( $Self->{DBObject}->Quote($Params->{Term}) );
    }
    else {
        return undef;
    }
    ...
    push @Bind, \$Term;
    my $Ret = $Self->{DBObject}->SelectAllArrayref(
        SQL      => $SQL,
        Bind     => \@Bind,
        Slice    => {},
