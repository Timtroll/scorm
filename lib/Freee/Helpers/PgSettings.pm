...
sub register {
    ...
    # запись данных из mock в таблицу settings
    $app->helper( '_add_mock' => sub {
        my $self = shift;

        my ( $sth, $res, $id, $sub, $resp, @mess);

        # проверка того, что таблицы settings не существует
        $sth = $self->pg_dbh->prepare( q( SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='settings' ) );
        $res = $sth->execute();
