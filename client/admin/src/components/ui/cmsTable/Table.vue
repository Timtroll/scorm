...
export default {
  ...
  computed: {
    ...
    editPanel_api () {return this.$store.getters.editPanel_api},
    ...
    removable () {
      return (this.table && this.table.settings && this.table.settings.removable)
        ? this.table.settings.removable
        : 0
    },

    editable () {
      return (this.table && this.table.settings && this.table.settings.editable)
        ? this.table.settings.editable
        : 0
    },
    ...
    tableRowsField () {
      ...
      if (!this.table.body.length) return
      return this.table.body[0]
    },
  ...
  methods: {
      ...
      if (this.tableRowsField) {
        ...
        for (let prop in row) {
          if (row.hasOwnProperty(prop)) {
            const index = protoLeaf.findIndex(i => i.name === prop)
            if (index >= 0) {
              header.push(protoLeaf[index])
            }
          }
        }
        return header
    ...
    notCheckedAll () {this.checked = false},

    // выделить все строки
    checkedAll () {this.checked = !this.checked},

    // Очистка поля поиска
    clearSearchVal () {this.searchInput = null},
