...
$result = {
    "data" => {
        "label" =>  "уроки",
        "current" =>  {
            "route"  => "/lesson",        # роут для получения детей
            "add"    => "/lesson/add",    # разрешает добавлять детей
            "edit"   => "/lesson/edit",   # разрешает редактировать детей
            "delete" => "/lesson/delete", # разрешает удалять детей
        },
        "child" =>  {
            "add"    =>    "/lesson/add",  # разрешает добавлять дочерние темы
        },
        "list" => [
            {
                "folder"      => 0,
...
$t->post_ok( $host.'/lesson/' => {token => $token} );
unless ( $t->status_is(200)->{tx}->{res}->{code} == 200  ) {
    diag("Can't connect \n");
    last;
}
