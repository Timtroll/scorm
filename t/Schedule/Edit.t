...
$test_data = {
    ...
    2 => {
        ...
        'result' => {
            'message'   => "Could not get Schedule '404'",
            'status'    => 'fail'
        },
        'comment' => 'Wrong id:' 
    },
    3 => {
        'result' => {
            'message'   => "/schedule/edit _check_fields: didn't has required data in 'id' = ''",
            'status'    => 'fail'
        },
        'comment' => 'No data:' 
    },
    4 => {
        ...
        'result' => {
            'message'   => "/schedule/edit _check_fields: empty field 'id', didn't match regular expression",
            'status'    => 'fail'
        },
...
foreach my $test (sort {$a <=> $b} keys %{$test_data}) {
    ...
    $t->post_ok( $host.'/schedule/edit' => {token => $token}  => form => $data );
    unless ( $t->status_is(200)->{tx}->{res}->{code} == 200  ) {
        diag("Can't connect \n");
        last;
    }
