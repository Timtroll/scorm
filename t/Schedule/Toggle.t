...
$test_data = {
    ...
    2 => {
        ...
        'result' => {
            'message'   => "/schedule/toggle _check_fields: didn't has required data in 'value' = ''",
            'status'    => 'fail'
        },
        'comment' => 'No value:'
    },
    3 => {
        ...
        'result' => {
            'message'   => "/schedule/toggle _check_fields: didn't has required data in 'id' = ''",
            'status'    => 'fail'
        },
        'comment' => 'No id:' 
    },
    4 => {
        ...
        'result' => {
            'message'   => "/schedule/toggle _check_fields: didn't has required data in 'fieldname' = ''",
            'status'    => 'fail'
        },
...
foreach my $test (sort {$a <=> $b} keys %{$test_data}) {
    ...
    $t->post_ok( $host.'/schedule/toggle' => {token => $token}  => form => $data );
    unless ( $t->status_is(200)->{tx}->{res}->{code} == 200  ) {
        diag("Can't connect \n");
        last;
    }
