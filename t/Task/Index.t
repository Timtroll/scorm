...
foreach my $test (sort {$a <=> $b} keys %{$test_data}) {
    ...
    $t->post_ok( $host.'/task/save' => {token => $token} => form => $data );
    unless ( $t->status_is(200)->{tx}->{res}->{code} == 200  ) {
        diag("Can't connect \n");
        last;
    }
...
$result = {
    "data" => {
        "label" =>  "Задания",
        "current" =>  {
            "route"  => "/task",        # роут для получения детей
            "add"    => "/task/add",    # разрешает добавлять детей
            "edit"   => "/task/edit",   # разрешает редактировать детей
            "delete" => "/task/delete", # разрешает удалять детей
        },
        "child" =>  {
            "add"    =>    "/task/add",  # разрешает добавлять дочерние темы
        },
        "list" => [
            {
                "folder"      => 0,
...
$t->post_ok( $host.'/task/' => {token => $token} );
unless ( $t->status_is(200)->{tx}->{res}->{code} == 200  ) {
    diag("Can't connect \n");
    last;
}
